...
if __name__ == '__main__':
    ...
    for i in range(nb):
        ...
        msk_looked = 'shadowMask_%02d_%drlks_%dalks.rdr' % (i+1,inps.rlks,inps.alks)
        cmd = "{}/look.py -i {} -o {} -r {} -a {}".format(SCR_DIR,
            os.path.join(inps.gdir, 'shadowMask_%02d.rdr' % (i+1)), 
            msk_looked,
            inps.rlks,
            inps.alks)
        runCmd(cmd)
        ...
        runCmd(cmd)
        print(script_dir)
        msk_looked_geo = 'shadowMask_%02d_%drlks_%dalks.cor.geo' % (i+1,inps.rlks,inps.alks)
        cmd = "{}/geo_with_ll.py -input {} -output {} -lat {} -lon {} -bbox \"{}\" -ssize {} -rmethod {}".format(
            script_dir,
            msk_looked, 
            msk_looked_geo,
